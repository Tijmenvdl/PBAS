"""SDVRP for AH weekly delivery scheduling — fast, simple version.

Key changes vs. the original:
  1. A working `greedy_warm_start` (nearest-neighbour) that correctly handles
     the heterogeneous fleet (weights are indexed by truck type, trucks have
     store-level restrictions, demand can be split across trucks).
  2. The greedy is ALSO used to size the fleet: K_t[t] = greedy_usage[t] + 2.
     This keeps the fleet just big enough for feasibility and cuts the number
     of binary variables by ~5× (185–279 trucks/day down to ~41–57).
  3. `model.optimize()` is called ONCE, AFTER the warm start is applied.
  4. kNN neighbours reduced from 20 to 10 — roughly halves the arc set.
  5. Output is guarded against the no-solution case.
"""
